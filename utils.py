...
project_name = data.get("project_name", "report").replace(" ", "_")
pdf_filename = f"output/{project_name}_{timestamp}.pdf"

# 創建 PDF 文件
doc = SimpleDocTemplate(pdf_filename, pagesize=A4, rightMargin=1*cm, leftMargin=1*cm, topMargin=1*cm, bottomMargin=1*cm)
...
# 建立 PDF
pdf_filename = "output_inspection.pdf"
try:
    doc.build(elements)
    print(f"PDF 報表已生成: {pdf_filename}")
